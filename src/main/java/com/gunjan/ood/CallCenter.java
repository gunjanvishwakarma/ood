...
public class CallCenter {
  public static void main(String[] args) {
    ...
    callManager.addRespondant(new Employee("R1", callManager));
    callManager.addRespondant(new Employee("R2", callManager));
    callManager.addRespondant(new Employee("R3", callManager));

    callManager.addManager(new Employee("M1", callManager));
    callManager.addManager(new Employee("M2", callManager));
    callManager.addManager(new Employee("M3", callManager));

    callManager.addDirector(new Employee("D1", callManager));
    callManager.addDirector(new Employee("D2", callManager));

    Caller foo = new Caller("foo");

    callManager.dispatch(foo); // should be R1
    callManager.dispatch(new Call()); // should be R2
    callManager.dispatch(new Call()); // should be R3
    callManager.dispatch(new Call()); // should be M1
  }
}
